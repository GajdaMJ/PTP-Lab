...
def PBR_model(T,fv1,fv2, V=131, tspan = [0,3600], n=6, teval=np.linspace(0,3600,500)):
    ...
    sol_me = scipy.integrate.solve_ivp(der_func, tspan, xini, t_eval=teval, args=(params, n)) 
    return sol_me
...
if __name__ == '__main__':
    ...
    # Colors for each tank's plot
    colors = ['firebrick', 'steelblue', 'forestgreen']  
    tank_data = [2, 5, 8]

    # Loop to plot the real temperature and model prediction for Tanks 2, 5, and 8 on the left plot (ax1)
    for t in range(len(tank_data)):
        ...
        temp_data_residual = temp_data[10:-1]
        # Plotting real temperature data on ax1
        ax1.plot(elapsed_time, (slopes[i]*temp_data + y_intercepts[i]) - (slopes[i]*temp_data[0] + y_intercepts[i]) + initial_temperature,  color=colors[t], label=f'Real Data, Probe {i+1}', linestyle='dashed', linewidth=2)

        
        # Plotting model prediction for corresponding temperature probe on ax1
        ax1.plot(sol_me.t / 60, sol_me.y[3 + tank_data[t] * 5, :] - 273.15, color=colors[t], label=f'Model Prediction, Tank {tank_data[t] + 1}', linewidth=2)


        interpolator = scipy.interpolate.interp1d(sol_me.t/60, sol_me.y[3 + tank_data[t] * 5, :] - 273.15, kind='linear')

        # Interpolate the model solution onto the time points of the temperature data
        sol_interpolated = interpolator(elapsed_time[10:-1])

        # Now you can calculate the residuals
        residualy = ((slopes[i]*temp_data_residual + y_intercepts[i]) - (slopes[i]*temp_data[0] + y_intercepts[i]) + initial_temperature) - sol_interpolated

        ax2.plot(elapsed_time[10:-1], residualy, color=colors[t], label=f'Residual, Probe {i+1}', linewidth=2)
    # Customize the left plot (ax1)
    ax1.set_title('Temperature Comparison for Tanks 2, 6, and 9', fontsize=16, fontweight='bold')
    ax1.set_xlabel('Elapsed Time (min)', fontsize=12)
    ax1.set_ylabel('Temperature (°C)', fontsize=12)
    ax1.set_xlim(0, 20)
    ax1.minorticks_on()
    ax1.legend(fontsize=10)
    ax1.grid(which='major', linewidth=2)
    ax2.axhline(0, color='black', linestyle='--', linewidth=2)  # Add a horizontal line at 0 for reference
    ax1.grid(which='minor', linewidth=0.3)
    # Right plot (ax2) - Placeholder code for future plotting
    # This is where you can add the desired plot for the right subplot
    ax2.set_title('Residuals of Temperature', fontsize=16, fontweight='bold')
    ax2.set_xlabel('Elapsed Time (min)', fontsize=12)
    ax2.set_ylabel('Residuals (°C)', fontsize=12)

    # Customize right plot (ax2) if needed
    ax2.set_xlim(0, 20)  # Example limits, adjust as needed
    ax2.minorticks_on()
