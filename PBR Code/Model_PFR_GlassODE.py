...
def der_func(t,C, parameters, n=6):
    ...
    for i in range(5*n):
        if i < 5:  # For the first set of concentrations and temperatures
           ...
           dcdt[3] = (total_flow / V) * (inlet_temp - C[3]) - H / (rho_effective * cp_effective) * C[0] * C[1] * k0 * np.exp(-Ea / (R * C[3])) + (U * A) / (rho_glass * cp_glass * V) * (C[i+1] - C[3])  # Reactor temperature derivative
           ...
           dcdt[4] = (U * A) / (rho_glass * cp_glass * V) * (C[3] - C[4])  # Temperature change of glass beads
            ...
            if np.mod(i, 5) == 0:
                ...
                dcdt[i] = (total_flow / V) * (C[i - 5] - C[i]) - H / (rho_effective * cp_effective) * C[i - 3] * C[i - 2] * k0 * np.exp(-Ea / (R * C[i])) + (U * A) / (rho_glass * cp_glass * V) * (C[i+1] - C[i]) 
            elif np.mod(i, 5) == 4:
